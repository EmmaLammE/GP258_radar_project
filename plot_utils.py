...
def mplot2_std(Pred_grid_xy, pred, std, rows, cols, title1, title2):    
    ...
    xmin = Pred_grid_xy[:,0].min(); xmax = Pred_grid_xy[:,0].max()
    ymin = Pred_grid_xy[:,1].min(); ymax = Pred_grid_xy[:,1].max()
    predmin = pred.min();predmax = pred.max();
    stdmin = std.min();stdmax = std.max();

    fig, [ax1, ax2] = plt.subplots(1, 2, figsize=(10,5))

    im = ax1.pcolormesh(x_mat, y_mat, pred_mat, vmin=predmin, vmax=predmax, cmap='gist_earth')
    ...
    # make colorbar
    cbar = make_colorbar(fig, im, predmin, predmax, 'Bed [m]', ax=ax1)

    im = ax2.pcolormesh(x_mat, y_mat, std_mat, vmin=stdmin, vmax=stdmax, cmap='gist_earth')
    ...
    # make colorbar
    cbar = make_colorbar(fig, im, stdmin, stdmax, 'Bed [m]', ax=ax2)
